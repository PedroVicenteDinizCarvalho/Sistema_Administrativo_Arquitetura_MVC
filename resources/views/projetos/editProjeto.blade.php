...
@section('content')
	...
	<div class="col-md-6 well">
		<form action="{{ url('home/projetos/update') }}" method="POST">
			{{ csrf_field() }}
			...
			<div class="form-group col-md-12 {{ $errors->has('tipo') ? 'has-error' : ''}}">
				...
				<input type="text" name="tipo" id="tipo" class="form-control" placeholder="Site, Software, Consultoria, Design Gráfico" value="{{ $projeto->tipo }}">
				@if($errors->has('tipo'))
					{{ $errors->first('tipo') }}
				@endif
			</div>
			<div class="form-group col-md-12 {{ $errors->has('nome') ? 'has-error' : ''}}">
				...
				<input type="text" name="nome" id="nome" class="form-control" placeholder="Nome do Projeto" value="{{ $projeto->nome }}">
				@if($errors->has('nome'))
					{{ $errors->first('nome') }}
				@endif
			</div>
			<div class="form-group col-md-12 {{ $errors->has('prazoEntrega') ? 'has-error' : ''}}">
				...
				<input type="number" name="prazoEntrega" id="prazoEntrega" class="form-control" placeholder="1" max="42" value="{{ $projeto->prazoEntrega }}">
			...
			<div class="form-group col-md-12 {{ $errors->has('valor') ? 'has-error' : ''}}">
				...
				<input type="number" name="valor" id="valor" class="form-control" placeholder="1000" value="{{ $projeto->valor }}">
				@if($errors->has('valor'))
					{{ $errors->first('valor') }}
				@endif
			</div>	
				...
				<div class="form-group col-md-12">
					...
					<input type="number" name="parcelasPagamento" id="parcelasPagamento" class="form-control" placeholder="(1 Caso seja à vista)" max="42"   value="{{ $projeto->parcelasPagamento }}" Required>
				</div>
				...
				<input type="number" name="cliente_id" id="cliente_id" class="form-control"  value="{{ $projeto->cliente_id}}" Required>
			</div>
